...
class YOLOF(DenseDetector):
    ...
    def forward_training(self, images, features, predictions, gt_instances):
        ...
        return self.losses(anchors, pred_logits, gt_labels, 
    ...
    def losses(self, anchors, pred_logits, gt_labels, pred_anchor_deltas, gt_boxes):
        ...
        Args:
            ...
            pred_logits, pred_anchor_deltas: both are list[Tensor]. Each element
                in the list corresponds to one level and has shape (N, Hi * Wi * Ai, K or 4).
                Where K is the number of classes used in `pred_logits`.

        Returns:
            dict[str, Tensor]: mapping from a named loss to a scalart tensor storing
                the loss.
        ...
        gt_labels_target = F.one_hot(gt_labels[valid_mask], num_classes=self.num_classes + 1)[
            :, :-1
        ]  # no loss for the lass (background) class
    ...
    """
    torch.no_grad()
        -> Any operations performed within that function or block will not have 
            their gradients computed or stored. This can help reduce memory usage 
            and computation time, especially when performing inference on a 
            trained model where gradients are not needed. 
    """
    @torch.no_grad()
    def label_anchors(self, anchors, gt_instances):
        ...
        Args:
            ...
            gt_instances (list[Instances]): a list of N `Instances`s. The i-th `Instances`
                contains the ground-truth per-instance annotations for the i-th input image.

        Returns:
            list[Tensor]: list of #img tensors. i-th element is a vector of labels whose length
                is the total number of anchors across all feature maps (sum(Hi * Wi * A)).
    ...
    def inference_single_image(
        ...
        Arguments:
            anchors (list[Boxes]): list of #feature levels. Each entry contains a Boxes
                object, which contains all the anchors in that feature level.
            box_cls (list[Tensor]): list of #feature levels. Each entry contains tensor
                of size (H x W x A, K).
            box_delta (list[Tensor]): same shape as 'box_cls' except that K becomes 4.
            image_size (type(H, W)): a tuple of the image height and width.
...
class YOLOFHead(nn.Module):
    """
    The head used in YOLOF for object classification and box regression.
    It has two subnets for the two tasks, with a common structure but seperate parameters. 
    """
